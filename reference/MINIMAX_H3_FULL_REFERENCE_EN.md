...
Every standalone `<Audio N>` definition refers to the real caller-assigned upload slot. Keep the exact filename in a separate operator checklist; never use a filename as model-facing binding prose. Before writing prompts, lock each upload slot one-to-one to a named target character or explicit narrator/off-screen source. A mixed/shared-track exception requires explicit user approval and unambiguous speaker/time ranges. When an `<Audio N>` explicitly corresponds to a target speaker, reuse that speaker's global ID in the definition: write `<Subject N> (Sx)` when the speaker maps to a defined subject, or use a stable voice description followed by `(Sx)` otherwise. The ID comes from the target video's global speaker order and is not independently assigned or renumbered in the audio definition. See Section 5.4 for the speaker-numbering rules:

```text
<Audio 1> is the voice-timbre and speaking-delivery reference for <Subject 1> (S1).
```

When one audio asset serves multiple roles for the same locked source, describe those roles in one natural sentence rather than creating additional subsections. Preserve the upload slot, target source, reuse/reference mode, retention marker, scope, and exclusions from the approved `AUDIO_REFERENCE_MAP` across all six sections. Keep the filename only in the operator checklist; mirror the same filename-free `<Audio N>` relationship in the paired Chinese prompt's compact binding line and owning vocal event. If no audio reference was requested, do not invent `<Audio N>` or audio task types.
...
<Audio 1> is the voice-timbre and speaking-delivery reference for <Subject 3> (S1), containing a spoken English vocal layer.
